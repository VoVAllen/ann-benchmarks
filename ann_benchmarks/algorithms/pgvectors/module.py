...
class PGVectoRs(BaseANN):
    ...
    def fit(self, X):
        ...
        conn = psycopg.connect(user="postgres", password="mysecretpassword", dbname="postgres", host="localhost", port=5432, autocommit=True)
        cur = conn.cursor()
        self._cur = cur
        ...
        if self._metric == "angular":
            cur.execute(
                """
                CREATE INDEX ON documents USING vectors (embedding cos_ops)
                WITH (options = $$
                capacity = 2097152
                [vectors]
                memmap = "ram"
    ...
    def set_query_arguments(self, ef_search):
        ...
        self._cur.execute("SET vectors.k = %d" % ef_search)
    ...
    def __str__(self):
        return f"Pgvecto.rs(m={self._m}, ef_construction={self._ef_construction}, ef_search={self._ef_search})"
